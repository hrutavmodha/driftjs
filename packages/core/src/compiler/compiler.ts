...
export class DriftJSCompiler {
  ...
  private updates: { nodeIdx: number, reg: number, thunkIdx: number, depMask: number, attrKeyIdx?: number }[] = [];
  ...
  private createThunk(code: string): Function {
    const thunk = new Function('regs', code);
  ...
  private compileScript(scriptNode: any): number {
    ...
    // First pass: Discover all state variables, functions, and classes
    walk(jsAst as any, {
      enter: (node: any) => {
        if (
          (node.type === 'VariableDeclarator' && node.id && node.id.type === 'Identifier') ||
          (node.type === 'FunctionDeclaration' && node.id && node.id.type === 'Identifier') ||
          (node.type === 'ClassDeclaration' && node.id && node.id.type === 'Identifier')
        ) {
  ...
  public compile(): CompiledProgram {
    ...
    for (const up of this.updates) {
      this.emit(Opcodes.EXEC_THUNK_GUARDED, up.reg, up.thunkIdx, up.depMask);
      if (up.attrKeyIdx !== undefined) {
        this.emit(Opcodes.SET_ATTRIBUTE, up.nodeIdx, up.attrKeyIdx, up.reg);
      } else {
  ...
  private compileElement(node: any): number {
    ...
    for (const [key, value] of Object.entries(node.attributes)) {
      ...
      if (valStr.startsWith('{') && valStr.endsWith('}')) {
        ...
        this.updates.push({ nodeIdx, reg, thunkIdx, depMask, attrKeyIdx: keyIdx });
      } else {
